...
names(Properties)[1:9] <- c(
  "MNA", "ExampleExtProp", "Result", 
  "EnvNum", "Space", "Dist", 
  "ImmigrationRate", "ExtirpationRate", "ExtPropNumAND.RData"
)
...
Properties <- Properties %>% dplyr::mutate(
  ExtPropNumAND.RData = gsub(pattern = patternString,
                                  replacement = "_",
                                  x = ExtPropNumAND.RData, perl = TRUE),
  EnvNum = gsub(pattern = patternString,
                replacement = "_",
                x = EnvNum, perl = TRUE)
) %>% tidyr::separate(
  ExtPropNumAND.RData, into = c("ExtPropNum", ".RData"),
  sep = "[.](?=[R])"
) %>% tidyr::separate(
  EnvNum, into = c("Env", "Environments"),
  sep = "[_]"
)  %>% tidyr::separate(
  ExtPropNum, into = c("ExtProp", "ExtirpationProportion"),
  sep = "[_]"
) %>% dplyr::select(
  -MNA, -ExampleExtProp, -Result, -.RData, -Env, -ExtProp
) %>% dplyr::mutate(
...
Diversity <- Diversity %>% dplyr::mutate(
  Measurement2 = dplyr::case_when(
    Measurement == "Jaccard" ~ "Jaccard Dist.",
...
PLOT_BALT <- ggplot2::ggplot(
  Diversity %>% dplyr::filter(
    Measurement2 %in% c("Jaccard Dist.", "Local Rich.", "Regional Rich."),
    Environment != "Mean"
  ),
  ggplot2::aes(
    x = Time,
  ...
  . ~ factor(
    ...
    levels = c("Local Rich.", "Regional Rich.", "Jaccard Dist.")
...
ggplot2::ggsave(
  filename = "MNA-Image-Example-Presence-ExtProp0.9.png",
  plot = objalt,
