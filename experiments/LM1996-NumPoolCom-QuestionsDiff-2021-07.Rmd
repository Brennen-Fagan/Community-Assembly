...
We will also need to recreate code from the file `LM1996-NumPoolCom-FoodWebs-2021-07.Rmd`.
```{r createGraphs}
foodWebs <- list()
...
#### Example LM 2
```{r gallery2}
toPostMortem(foodWebs[[34]], nodeSize = "None", threshold = "Adaptive") -> temp
```

#### Invadable
```{r gallery3}
toPostMortem(foodWebs[[43]], nodeSize = "None", threshold = "Adaptive") -> temp
```

#### Does Not Persist
```{r gallery4}
toPostMortem(foodWebs[[46]], nodeSize = "None", threshold = "Adaptive") -> temp
```

#### Hybrid
```{r gallery5}
toPostMortem(foodWebs[[42]], nodeSize = "None", threshold = "Adaptive") -> temp
```

#### Largest Assembled
```{r gallery6}
toPostMortem(foodWebs[[33]], nodeSize = "None", threshold = "Adaptive") -> temp
```

#### Largest Hybrid
```{r gallery7}
toPostMortem(foodWebs[[49]], nodeSize = "None", threshold = "Adaptive") -> temp
```
...
```{r}
matsPerturbationsProps <- do.call(rbind, 
                                  lapply(matsEffectsAdd, function(m) {
  if (is.null(m)) return(
    data.frame(
...
matsPerturbationsPropsByType <- do.call(
  rbind, 
  lapply(seq_along(matsEffectsAdd), function(i, permat, commun, commat) {
    #print(i)
...
for (i in 1:length(foodWebs)) {
  ...
  Leontief[[i]] <- with(
    foodwebsThe, {
      flow.mat <- matrix(data = 0, nrow = nrow(Vertices), ncol = nrow(Vertices))
      rownames(flow.mat) <- colnames(flow.mat) <- Vertices$node
      if (!is.null(Edges)) {
        for (link in 1:nrow(Edges)) {
          linkRow <- Edges[link, ]
          # NOTE: from-row-to-column
          speciesThe <- c(which(Vertices$node %in% linkRow$from),
                          which(Vertices$node %in% linkRow$to))
          flow.mat[linkRow$from, linkRow$to] <-
            linkRow$effectPerUnit * 
            Vertices$N[speciesThe[1]] * Vertices$N[speciesThe[2]] * 
            Vertices$M[speciesThe[2]]
        ...
        #print(flow.mat)
        stopifnot(
          # Effect on resource (-) should be larger than effect on consumer (+).
          # Note: diag trivially satisfies for consumers (0 <= abs(-val)).
          #       Unfortunately, not for basals.
          ifelse(flow.mat > 0, abs(flow.mat), 0) - t(ifelse(flow.mat < 0, abs(flow.mat), 0)) - diag(diag(flow.mat)) <= 0
        )
      }
      ...
      flow.input <- ifelse(
        Vertices$Type == "Basal",
        Vertices$M * Vertices$ReproductionRate * Vertices$N,
        0
      )
      ...
      flow.export <- ifelse(
        Vertices$Type == "Basal",
        Vertices$N^2 * Vertices$Intraspecific * Vertices$M,
        Vertices$ReproductionRate * Vertices$N * Vertices$M
      )
      
      flow.output <- flow.respiration + flow.export
      
      storage <- Vertices$N
      living <- rep(TRUE, nrow(Vertices))
...
save(
  ...
  file = "LM1996-NumPoolCom-QOutDif-2021-07.RData"
