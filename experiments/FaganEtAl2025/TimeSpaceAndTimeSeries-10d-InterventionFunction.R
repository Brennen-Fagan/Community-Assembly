...
handlerID <- function(
  ...
  # speciesAffinityDictionaryChoice, patchAffinityDictionaryChoice,
  # Date (YYYY-MM-DD)
) {
  # ID Handling: ##############################################################
  ...
  if (!is.null(names(ID))) { # List/data.frame of components.
    ...
    datfile <- with(
      ID, paste0(
        ...
        speciesAffinitySeedChoice, "-", patchAffinitySeedChoice,
        ".RData")
    )
  ...
  } else if (length(ID) <= 15 && length(ID) >= 13) { # Unnamed list of components
    #TODO Double check that adjusted correctly.
    runDictionary0 <- 0
    ...
    if (length(ID) > 13) {
      ...
      if(length(ID) == 15 && !runDictionaryDateTest) {
        stop("ID has 15 entries, but last not parsed as %Y-%m-%d date.")
      }
      if (length(ID) == 15 || !runDictionaryDateTest) {
        # == 14 entries and last is not a date (X)or have 15 entries.
    ...
    tryCatch({
      ...
      datfile <- with(
        ID, paste0(
          ...
          # speciesAffinityDictionaryChoice, "-", 
          # patchAffinityDictionaryChoice, "_",
          ID[[runDictionary0 + 11]], "-", ID[[runDictionary0 + 13]], "_",
          # SEEDS:
          # poolpatchSeedChoice, "-", dynamicsSeedChoice, "-",
          # eventsSeedChoice, "-", initialConditionsSeedChoice, "-",
          # speciesAffinitySeedChoice, "-", patchAffinitySeedChoice
          ...
          ID[[runDictionary0 + 12]], "-", ID[[runDictionary0 + 14]],
          ".RData")
      )
...
interventionWrapper <- function(
  ...
  seedsMain = data.frame(# runif(2)*1e8 # [1] 95756357 68101170
    "patches"  = 95756357,
    "times"    = 68101170
  ),
  returnResults = FALSE,
