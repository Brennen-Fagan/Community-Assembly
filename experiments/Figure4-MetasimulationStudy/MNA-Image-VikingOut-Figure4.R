...
# # Borrow mostly from the helper function for Statistics over Space.
# obj <- ggplot2::ggplot(
#   concludingplotdata ,
...
# ) + ggplot2::geom_line(
#   data = concludingplotdata %>% dplyr::group_by(
...
# #   name = "", values = c("cyan", "magenta", "goldenrod4")
...
# ) + ggplot2::facet_wrap(~FacetPanel, scales = "free", ncol = 1)
...
# textdispersal <- ggplot2::ggplot(
# ) + ggplot2::geom_point(
#   data = data.frame(
#     Dispersal = unique(concludingplotdata$Dispersal),
#     height = 1 + (0:10)/100
#   ),
#   mapping = ggplot2::aes(
#     x = Dispersal, y = height
#   ),
#   color = "transparent", inherit.aes = FALSE
# ) + ggplot2::annotate(
#   "text", x = c(1.6, 10.4), y = c(1.05, 1.05),
#   size = 4,
#   label = c("No Dispersal", "Full Dispersal")
# ) + ggplot2::theme_bw(
# ) + ggplot2::coord_cartesian(
...
doublearrow <- ggplot2::ggplot(
  data.frame(
    Dispersal = range(concludingplotdata$Dispersal),
    height = 1,
    Dispersalend = rev(range(concludingplotdata$Dispersal))
  ),
  ggplot2::aes(
    x = Dispersal,
    xend = Dispersalend,
  ...
  "text", x = c(1.5, 10.5), y = c(20, 20),  size = 4,
  label = c("No Dispersal", "Full Dispersal")
) + ggplot2::theme_bw(
) + ggplot2::scale_x_discrete(
  name = #"Inter-community resistance and unit proportion dispersing",
...
ggplot2::ggsave(
  ...
  plot = (
    objs[[1]]/objs[[2]]/objs[[3]] + #/doublearrow +
