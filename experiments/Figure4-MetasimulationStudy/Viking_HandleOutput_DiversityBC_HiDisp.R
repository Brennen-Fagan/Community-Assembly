...
filesParameters <- dir(path = "Prepared_2022-05-03", pattern = "^MNA[-]HiDisp.+Cases[.]csv$",
                       full.names = TRUE, recursive = TRUE,
                       include.dirs = TRUE, no.. = TRUE)
parametersMaster <- load(file.path("Prepared_2022-05-03", "MNA-HiDispParameters.RData"))
...
results <- foreach::foreach(
  ...
  fileName <- file.path(
    outputLocation,
    paste0("MNA-HiDisp", LETTERS[idNums[1]], "-Cases-DiversityBC-",
  ...
  tryCatch({
    ...
    # This is split between MNA-HiDispParameters.RData and the .csv files.
...
write.csv(results_missing,
          file = file.path(outputLocation,
                           paste0("MNA-HiDispMissingRuns.csv"))
)
write.csv(results_error,
          file = file.path(outputLocation,
                           paste0("MNA-HiDispErrorRuns.csv"))
)
