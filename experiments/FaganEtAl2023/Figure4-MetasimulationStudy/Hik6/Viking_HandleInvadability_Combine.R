...
inputLocation <- c(
  ...
  file.path(directory, "SaveInvadability_2023-03-06")
)
...
success <- lapply(
  ...
  function(files) {
    ...
    # parallel::clusterEvalQ(clust, .libPaths(allLibraryPaths))
    ...
    save(Attributes,
         Invadabilities,
         file = paste0("Set", setNumber+3, "-AllInvadabilityData.RData"),
         compress = "bzip2"
    )
