# Based on the below files. Note that we are sampling the entirety of the no
# intervention case though in order to visualise the sampled richness.
...
### Quantities: ###############################################################
# When:
samplingQuantity <- 1000 # Not guaranteed! Number of samples taken by a run
samplingTimeScaleLogarithmic <- FALSE # Logarithmically spread from intervention
...
samplingSeed <- switch(
  datfolder,
  "TSTS_Simulations_18-1_6-6_2024-05-23" = 22081654
)
...
samples <- foreach::foreach(
  ...
  fileproperties[[1]][[2]] <- "SamplingBES"
  filename <- file.path(
    dirname(file),
  ...
  results$Ellipsis$TimeSpan <-
    ...
    results$Abundance[nrow(results$Abundance), 1]*.95 # Make sure rows -> right
