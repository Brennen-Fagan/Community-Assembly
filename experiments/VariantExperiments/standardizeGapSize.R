standardizeGapSize <- function(abundance, gap, epsilon = NULL) {
...
results <- lapply(dir(datfolder, "Simulation", full.names = T), function(x) {nm <- load(x); return(get(nm))})
preferredGap <- quantile(unlist(lapply(results, function(r) diff(r$Abundance[, 1]))), p = 0.03)
temp <- lapply(
  seq_along(results),
