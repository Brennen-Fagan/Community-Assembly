...
names(Properties)[1:9] <- c(
  "Mutualism", "ExampleExtProp", "Result", "EnvNum",
...
Properties <- Properties %>% dplyr::mutate(
  ...
  -Mutualism, -ExampleExtProp, -Result, -.RData, -Env, -ExtProp
) %>% dplyr::mutate(
...
SpeciesPresence$Type <-
  PoolsMats[[1]]$Pool$Type[
...
PoolsMats[[1]]$Pool <-
  PoolsMats[[1]]$Pool %>% dplyr::arrange(
    ...
    SizeID = 1:nrow(PoolsMats[[1]]$Pool)
  )
...
SpeciesPresence <- SpeciesPresence %>% dplyr::group_by(
  Imm, Ext, ExtinctionProportion, Space, Distance,
  Species, Time, SizeID
) %>% dplyr::summarise(
  Count = n(), .groups = "drop"
) %>% dplyr::mutate(
...
DiversityRibbons <- Diversity %>% dplyr::filter(
  ...
  Time, Distance, Imm, Ext, ExtinctionProportion, Environments,
  Space, Dispersal,
  Measurement
...
PLOT_T_pre <- ggplot2::ggplot(
  SpeciesPresence %>% dplyr::mutate(
    Dispersal = dplyr::case_when(
      Space == min(Space) ~ "Full Dispersal",
      Space == max(Space) ~ "No Dispersal",
      Space == sort(unique(Space))[2] ~ "Med. Dispersal"
    )
  ),
  ...
  limits = c(0, nrow(PoolsMats[[1]]$Pool))
)
