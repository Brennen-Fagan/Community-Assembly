datfolders <- dir(pattern = "TSTS_Simulations_.+30.+2024-10-22$") # Regex
...
# Flatten one at a time rather than en masse. Attempting to reduce memory use.
while(length(diversities) > 0) {
  d <- diversities[[1]]
  ...
  if (!exists("diversitiesFlattened")) {
    diversitiesFlattened <- d$Diversity
  } else {
    diversitiesFlattened <- rbind(diversitiesFlattened, d$Diversity)
  }

  diversities <- diversities[-1]
}
