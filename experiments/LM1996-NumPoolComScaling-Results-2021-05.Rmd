...
for (grp in unique(candidateData$TotalID)) {
  ...
  if (nrow(candidateDataSubset) > 1) {
    ...
    newProductivities <- combn(
      candidateDataSubset$CommunityProd, 2, 
    )
    communities <- c(communities, newCommunities)
    productivities <- c(productivities, newProductivities)
    totalCommunities <- c(
      totalCommunities,
...
islandInteractionResults <- data.frame(
  ...
  Productivity1 = productivities[seq(from = 1, to = length(productivities), by = 2)],
  Community2 = communities[seq(from = 2, to = length(communities), by = 2)],
  Productivity2 = productivities[seq(from = 2, to = length(productivities), by = 2)],
...
islandInteractionResults %>% dplyr::mutate(
  ...
  ProdDiff = Productivity1 - Productivity2
