...
date: "Created 2021-03-23, last compiled `r Sys.Date()`"
...
```{r pools2}
system.time(pools2 <- foreach::foreach(
...
```
```{r commats2}
system.time(communityMats2 <- foreach::foreach(
  i = seq_along(pools2),
  s = seedsPrep2[
...
```{r table3runsv2} 
system.time(results2 <- foreach::foreach(
  i = 1 : (length(basal2) * length(consumer2))
) %:% foreach::foreach(
  ...
  if (LawMorton1996_CheckUninvadable(
    ...
    indices <- which(!is.na(aRun$Abundance) & aRun$Abundance > 2)[-1]
    return(list(list(
      Combn = toString(indices - 1),
      Popul = toString(round(aRun$Abundance[indices]))
      )
    ))
  } else {
    return(list(list(
...
endPoints <- lapply(results2, function(run) {
  combns <- which("Combn" == names(unlist(run)))
  table(unlist(run)[combns])
})
endPops <- lapply(results2, function(run) {
  combns <- which("Popul" == names(unlist(run)))
  table(unlist(run)[combns])
})
endPoints <- lapply(endPoints, function(x) {
  mutate(data.frame(x, stringsAsFactors = FALSE),
...
What do the networks that these communities describe actually look like?

```{r table3Networks}
library(igraph)
library(cheddar)
cheddarCommunities <- foreach::foreach(
  communities = iterators::iter(endPoints),
  matrices = iterators::iter(communityMats2),
  pools = iterators::iter(pools2)
) %:% foreach::foreach(
  community = iterators::iter(communities, by = 'row')
) %do% {
  print(community)
  ourSet <- as.numeric(unlist(strsplit(community$Var1, ', ', fixed = TRUE)))
  if (all(is.na(ourSet))) {
    return(NULL)
  }
  
  ourMatrix <- matrices[ourSet, ourSet]
  
  colnames(ourMatrix) <- paste0('s',as.character(ourSet))
  rownames(ourMatrix) <- colnames(ourMatrix)
  
  ourGraph <- igraph::graph_from_adjacency_matrix(
    ourMatrix, weighted = TRUE
  )
  
  ourGraph <- igraph::set.vertex.attribute(
    ourGraph, "name", value = colnames(ourMatrix)
  )
  
  ourPool <- pools[ourSet,]
  names(ourPool)[1] <- "node"
  ourPool$node <- colnames(ourMatrix)
  names(ourPool)[3] <- "M"
  
  # Add in the abundances while we're at it.
  ourAbund <- LawMorton1996_NumIntegration(
    A = ourMatrix,
    R = ourPool$ReproductionRate,
    X = rep(1, nrow(ourPool)),
    InnerTimeStepSize = 10,
    OuterTimeStepSize = 10000
  )
  while (!all(round(
    ourAbund[nrow(ourAbund), -1] / 
    ourAbund[nrow(ourAbund) - 1, -1], 2) == 1)
  ) {
    ourAbund <- LawMorton1996_NumIntegration(
      A = ourMatrix,
      R = ourPool$ReproductionRate,
  ...
  ourPool$N <- ourAbund[nrow(ourAbund), -1]
  
  ourGraphAsDataFrame <- as_data_frame(ourGraph)
    
  # cheddar does not like cannibals.
  ourGraphAsDataFrame <- ourGraphAsDataFrame[
    ourGraphAsDataFrame$to != ourGraphAsDataFrame$from,
  ]
  
  # Add in abundances for calculating abundance * (gain or loss)
  ourGraphAsDataFrame <- dplyr::left_join(
    ourGraphAsDataFrame,
    dplyr::select(ourPool, node, N),
    by = c("to" = "node")
  )
  
  # Split data frame.
  ourResCon <- ourGraphAsDataFrame[ourGraphAsDataFrame$weight > 0,]
  ourConRes <- ourGraphAsDataFrame[ourGraphAsDataFrame$weight < 0,]
  
  # Reorder and rename variables.
  ourResCon <- dplyr::select(ourResCon, 
                             resource = to, consumer = from, 
                             gainPerUnit = weight, resourceAbund = N)
  ourConRes <- dplyr::select(ourConRes, 
                             resource = from, consumer = to, 
                             lossPerUnit = weight, consumerAbund = N)
  
  ourResCon <- dplyr::mutate(dplyr::group_by(ourResCon, consumer),
                             gainEfficiency = gainPerUnit / sum(gainPerUnit),
                             gainActual = gainPerUnit * resourceAbund,
                             gainNormal = gainActual / sum(gainActual))
  ourConRes <- dplyr::mutate(dplyr::group_by(ourConRes, resource),
                             lossEfficiency = lossPerUnit / sum(lossPerUnit),
                             lossActual = lossPerUnit * consumerAbund,
                             lossNormal = lossActual / sum(lossActual))
  
  ourCommunity <- cheddar::Community(
    ourPool,
    properties = list(
      title = community$Var1,
      M.units = "masses",
      N.units = "abund"
    ),
    trophic.links = dplyr::full_join(ourResCon, ourConRes, 
...
invisible(lapply(cheddarCommunities, function(y) {
  lapply(y, function(x) {
    if (!is.null(x)) PlotWebByLevel(x, show.level.lines = TRUE, level = 'LongWeightedTrophicLevel')
  })
}))
```
...
```{r table7runsv2} 
system.time(results7 <- foreach::foreach(
