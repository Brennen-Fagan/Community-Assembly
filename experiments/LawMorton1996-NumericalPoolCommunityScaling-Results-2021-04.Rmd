...
temp <- names(unlist(paramFrame$EndStateSizes))
temp <- temp[temp != "0"]

plotScalingData <- data.frame(
  row = rep(paramFrame$CombnNum, paramFrame$EndStateSizesNum),
  Basals = rep(paramFrame$Basals, paramFrame$EndStateSizesNum),
  Consumers = rep(paramFrame$Consumers, paramFrame$EndStateSizesNum),
  CommunitySize = as.numeric(temp)
)
