...
Instead, we seem to observe a (time and environment averaged) value of 
`r mean(Diversity$Richness)`.
(If we consider the first 10,000 time units as burn-in, we instead see a value of
`r mean(Diversity$Richness[Diversity$Time > 10000])`.)
...
Indeed, only `r unique(EnvDiversity$Species[EnvDiversity$Richness > 5])` are 
ever present in more than 5 environments at once.
...
```
The amount of explained variation is `r sum(summary(PCA)$importance[2, ])`.
...
The system is still being reduced (from 100 Species * 10 Environments to 25 Components to cover `r sum(summary(PCA)$importance[2, ])` of the variation).
<!-- 
