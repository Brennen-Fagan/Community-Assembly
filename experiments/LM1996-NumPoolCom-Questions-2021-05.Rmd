...
```{r}
Leontief <- lapply(Leontief, function(x) {
  if (!is.null(x)) 
    enaR::enaMTI(enaR::balance(x, method = "AVG2"))$Relations.Table 
  else x
})
```
...
```{r}
lapply(Leontief, function(x) {if(!is.null(x)) table(x$`Mixed (integral)`)})
```

In contrast to earlier attempts, we now see a few rare mutualisms.
The algorithm is detecting them as true mutualisms (rather than "self-mutualism"), but I have not yet investigated what makes them mutualistic (or what the "integral" algorithm is).
