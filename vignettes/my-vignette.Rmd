---
title: "Law and Morton 1996: For comparison"
...
Notice that, despite continuing to add species in the simulation after time 1615, the populations remain largely the same.
...
seedsPrep <- runif(2 * length(basal2) * length(consumer2)) * 1E8
seedsRun <- runif(runs * length(basal2) * length(consumer2)) * 1E8
```
...
pools2 <- lapply(
  ...
  s = seedsPrep[1:(length(basal2) * length(consumer2))],
...
communityMats2 <- lapply(
  ...
  pools = pools2,
  parameters = parameters,
  s = seedsPrep[(length(basal2) * length(consumer2) + 1):(
...
runs <- lapply(
  # for each combination of numbers of basal and consumer species
  1 : (length(basal2) * length(consumer2)), 
  function(i, pools, communityMats, events, runs) {
    # run the system `runs` times
    replicate(
      n = runs,
      expr = {
        # taking the associated pool and community matrix,
        aRun <- LawMorton1996(
          Pool = pools[[i]], 
          CommunityMat = communityMats[[i]],
          IntegratorTimeStep = 100,
        ...
        # check if the result is uninvadable
        if (LawMorton1996_CheckUninvadable(
          AbundanceRow = aRun$Abundance,
          Pool = pools[[i]],
          CommunityMatrix = communityMats[[i]],
          Threshold = 2
        )) {
          # and, if so, store the result.
          return(which(!is.na(aRun$Abundance) & aRun$Abundance > 2))
        } else {
          return(NULL)
        }
      },
...
```{r tabulationRuns}
endPoints <- lapply(runs, function(run) {
  table(run)
})
```
